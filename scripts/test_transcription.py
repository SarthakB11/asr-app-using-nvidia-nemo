...
"""Test script to transcribe Hindi audio files using the ASR API."""
import os
import glob
import requests
import json
...
TEMP_DIR = Path("temp_audio_conversion_output")  # Directory for converted WAVs
...
def decode_unicode_text(text):
    ...
    # ▁ is a special token used by NeMo models for word boundaries
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for result in results:
            writer.writerow({
                ...
                'processing_time_ms': result['transcription'].get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Test ASR transcription on Hindi audio files'
    )
    parser.add_argument(
    ...
    mp3_files = glob.glob(
        os.path.join(HINDI_DATASET_PATH, "*.mp3")
    )[:args.num_files]
        ...
        if convert_to_wav(mp3_file, wav_file, target_sr=16000, channels=1):
            ...
            if transcription:
                ...
                if 'processing_time_ms' not in transcription:
                    transcription['processing_time_ms'] = round(
                        (end_time - start_time) * 1000, 2
                    )

                # Get the transcription text and decode it
                # The API response structure has 'transcription' nested inside
                ...
                ms = transcription.get('processing_time_ms')
                logger.info(f"Time: {ms} ms")
            else:
    ...
    logger.info("\n" + "="*50)
    logger.info(f"Results Summary: {len(results)}/{len(mp3_files)} files")
    logger.info("="*50)
    ...
    for i, result in enumerate(results, 1):
        ...
        logger.info(
            f"   Processing Time: "
            f"{result['transcription'].get('processing_time_ms', 'N/A')} ms"
        )
        logger.info("-"*50)
    ...
    if results:
        ...
        if args.format in ['json', 'both']:
            json_output_file = os.path.join(
                args.output_dir, f"transcription_results_{timestamp}.json"
            )
            ...
            logger.info(f"JSON saved to: {json_output_file}")

        # Save as CSV if requested
        if args.format in ['csv', 'both']:
            csv_output_file = os.path.join(
                args.output_dir, f"transcription_results_{timestamp}.csv"
            )
